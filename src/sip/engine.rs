...
impl ProxyEngine {
    ...
    pub async fn process_packet(
        ...
        if packet.is_request() {
                ...
                )
            {
                ...
                return Some((
                    SipResponseFactory::create_error(packet, 482, "Loop Detected"),
                    Some(src_addr),
                ));
            }
            ...
            if SipRouter::decrement_max_forwards(packet).is_err() {
                ...
                return Some((
                    SipResponseFactory::create_error(packet, 483, "Too Many Hops"),
                    Some(src_addr),
                ));
            }
            ...
            if packet.method != Method::Ack {
                ...
                let action = if let Some(tx) = self.transactions.get(&tx_key) {
                    ...
                    if let Some(new_tx) = SipTransaction::new(packet) {
                        self.transactions.insert(tx_key.clone(), new_tx);
                    }
    ...
    async fn handle_request(
        ...
        if in_dialog && packet.method != Method::Cancel {
            ...
            if let Some(real_peer) = self
                ._router
                .resolve_in_dialog_target(&call_id, real_src_ip)
                .await
            {
        ...
        let mut request =
            tonic::Request::new(sentiric_contracts::sentiric::sip::v1::GetNextHopRequest {
        ...
        match self.routing_logic.get_next_hop(request).await {
            Ok(res) => {
                ...
                let target_addr = if gateway_id == "internal-p2p"
                    || gateway_id == "direct-route-in-dialog"
                {
                    packet.uri = next_hop_uri.replace("<", "").replace(">", "");
                    self._router
                        .get_client_source(&call_id)
                        .await
                        .or(Some(src_addr))
                ...
                if let Some(target) = target_addr {
                    ...
                    self._router
                        .register_call_route(&call_id, real_src, real_dst)
                        .await;
    ...
    async fn handle_register(
            ...
            if let Some(digest) = crate::sip::auth::DigestAuth::parse(auth_val) {
                ...
                if let Some(clients) = clients_guard.as_ref() {
                    ...
                    let mut req = tonic::Request::new(
                        sentiric_contracts::sentiric::user::v1::GetSipCredentialsRequest {
                    ...
                    if !call_id.is_empty() {
                        if let Ok(meta_val) =
                            tonic::metadata::MetadataValue::try_from(call_id.as_str())
                        {
                    ...
                    match user_client.get_sip_credentials(req).await {
                        ...
                        Err(e) => {
                            warn!(event="SIP_AUTH_USER_FAIL", sip.call_id=%call_id, user=%digest.username, error=%e, "❌ Kullanıcı veritabanında bulunamadı.")
                        }
                    }
                }
            }
        ...
        if !is_authenticated {
            ...
            let www_auth = format!(
                ...
                self.config.sip_realm, nonce
            );
            resp.headers.push(Header::new(
                HeaderName::Other("WWW-Authenticate".to_string()),
                www_auth,
            ));
        ...
        if let Some(clients) = clients_guard.as_ref() {
            ...
            let mut req =
                tonic::Request::new(sentiric_contracts::sentiric::sip::v1::RegisterRequest {
            ...
            match reg_client.register(req).await {
                Ok(_) => {
                    ...
                    ok_resp
                        .headers
                        .push(Header::new(HeaderName::Contact, actual_contact_uri));
                    if expires > 0 {
                        ok_resp.headers.push(Header::new(
                            HeaderName::Other("Expires".to_string()),
                            expires.to_string(),
                        ));
                    }
                ...
                Err(e) => {
                    ...
                    return Some((
                        SipResponseFactory::create_error(packet, 403, "Forbidden"),
                        Some(src_addr),
                    ));
                }
            }
    ...
    async fn handle_response(
        ...
        if let Some(next_via) = packet.headers.iter().find(|h| h.name == HeaderName::Via) {
            if let Some(target) = SipRouter::resolve_response_target(
                &next_via.value,
                crate::sip::server::DEFAULT_SIP_PORT,
            ) {
                debug!(
