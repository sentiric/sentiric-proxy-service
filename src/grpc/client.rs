...
impl InternalClients {
    pub async fn connect(config: &AppConfig) -> Result<Self> {
        info!(
            event = "GRPC_CLIENTS_START",
            "🔌 İç servislere bağlanılıyor (mTLS + Lazy Connect)..."
        );
        ...
        if config.ca_path.is_empty() {
            error!(
                ...
                "CA_PATH tanımlanmamış. Sistem başlatılamaz."
            );
        ...
        let registrar_channel = connect_endpoint(
            &config.registrar_grpc_url,
            "sip-registrar-service",
            &tls_config,
        )
        .await?;
        let b2bua_channel =
            connect_endpoint(&config.b2bua_grpc_url, "sip-b2bua-service", &tls_config).await?;
        let dialplan_channel =
            connect_endpoint(&config.dialplan_grpc_url, "dialplan-service", &tls_config).await?;
        let user_channel =
            connect_endpoint(&config.user_service_grpc_url, "user-service", &tls_config).await?;

        info!(
            event = "GRPC_CLIENTS_READY",
            "✅ Tüm dış gRPC istemcileri yapılandırıldı (Lazy Mode)."
        );
