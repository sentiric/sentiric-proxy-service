...
impl App {
    ...
    pub async fn run(self) -> Result<()> {
        ...
        let grpc_server_handle = tokio::spawn(async move {
            ...
            GrpcServer::builder()
                ...
                .serve_with_shutdown(grpc_config.grpc_listen_addr, async {
                    ...
                    info!("gRPC sunucusu kapatma sinyali aldı.");
                })
                .await
                .context("gRPC sunucusu çöktü")
        });

        // 4. Bağlantı Yöneticisi (Arka Planda)
        let config_clone = self.config.clone();
        let clients_container_clone = clients_container.clone();
        
        tokio::spawn(async move {
            info!("⏳ Dış servislere (Dialplan, Registrar) bağlanılıyor...");
            loop {
                match InternalClients::connect(&config_clone).await {
                    Ok(c) => {
                        let mut guard = clients_container_clone.lock().await;
                        *guard = Some(c);
                        info!("✅ Dış servis bağlantıları sağlandı.");
                        break;
                    },
                    Err(e) => {
                        warn!("⚠️ Dış servis bağlantı hatası (Retry in 5s): {}", e);
                        tokio::time::sleep(Duration::from_secs(5)).await;
                    }
                }
            }
        ...
        tokio::select! {
            ...
            _ = ctrl_c => { warn!("Kapatma sinyali (Ctrl+C) alındı."); },
            _ = shutdown_rx.recv() => { warn!("Kapatma sinyali alındı."); }
        }

        // Graceful Shutdown Tetikleyicileri
